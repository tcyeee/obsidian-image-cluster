...
/**
 * 缩略图缓存路径：runtimeDefaults.thumbnailPath（用户可在设置里自定义） + md5(file.path + 目标分辨率)。
...
export function getThumbPath(filePath: string): string {
  return normalizePath(`${runtimeDefaults.thumbnailPath}${md5(`${filePath}@${getThumbnailTargetSide()}`)}`);
}
